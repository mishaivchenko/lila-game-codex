...
- Last updated: `2026-02-27T10:52:00Z`
- Generated by: `Codex`
...
- Production auth + admin-scope hardening:
  - app auth token now carries Telegram chat scope (`chatInstance`, `chatType`, `startParam`), allowing backend authorization to distinguish “user is admin” from “user may host in this specific chat”;
...
- TMA auth resilience:
  ...
  - frontend auth now tries both backend routes (`/api/auth/telegram/webapp` then `/api/auth/telegram`) and surfaces concrete backend error payloads instead of collapsing everything into generic “Telegram auth failed”;
  - when backend is temporarily unavailable (`502/503`), TMA bootstrap now enters a Telegram-runtime fallback using `initDataUnsafe.user` instead of hard-failing the whole app shell.
...
- Host/admin room flow expansion:
  ...
  - room state model now carries `activeCard`, host/player notes, room settings, and per-move dice arrays, enabling host mirror-view of player cards without allowing other players to close чужі cards;
...
- Regression coverage added:
  - backend tests now cover host-only room setting updates and the rule that only the host may close another player’s active card.
...
- Telegram Login + history foundations:
  ...
  - frontend Telegram shell now syncs active sessions to backend and Home page renders `Мої подорожі` list with resume/restart actions for Telegram-authenticated users;
...
- Journey setup contrast fix:
  - retokenized tab switcher (`Проста гра / Глибока гра / Правила гри`) to active/inactive theme vars, removing hardcoded light palette values;
  - retokenized locked `Глибока гра (Coming soon)` card and overlay container for readable text/background pairing across all themes.
...
| Source files | 172 |
| Source LOC | 16952 |
| Frontend files / LOC | 155 / 14069 |
| Backend files / LOC | 17 / 2883 |
| Test files / LOC | 38 / 2888 |
