...
export const attachHostRoomSocket = (server: HttpServer): Server => {
  ...
  namespace.on('connection', (socket) => {
    ...
    socket.on('rollDice', ({ roomId }: { roomId: string }) => {
      try {
        const result = rollDiceForCurrentPlayer({
          roomId,
          userId: authUser.id,
        });
    ...
    socket.on('updateNote', ({ roomId }: { roomId: string; cell: number; note: string }) => {
      try {
        const snapshot = recordRoomNote({ roomId, userId: authUser.id });
        namespace.to(`room:${roomId}`).emit('roomStateUpdated', snapshot);
      } catch {
    ...
    socket.on('hostCommand', ({ roomId, action }: { roomId: string; action: 'start' | 'pause' | 'resume' | 'finish' }) => {
      try {
        ...
        const snapshot =
          action === 'start'
            ? startRoom(roomId, authUser.id)
            : action === 'pause'
              ? setRoomStatus(roomId, authUser.id, 'paused')
              : action === 'resume'
                ? setRoomStatus(roomId, authUser.id, 'in_progress')
                : setRoomStatus(roomId, authUser.id, 'finished');
        namespace.to(`room:${roomId}`).emit('roomStateUpdated', snapshot);
      } catch {
    ...
    socket.on('disconnecting', () => {
      for (const roomName of socket.rooms) {
        ...
        try {
          const snapshot = setRoomConnectionState(roomId, authUser.id, 'offline');
